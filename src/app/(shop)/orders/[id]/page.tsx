...
import { IspaidComponent } from "./ui/IspaidComponent";
      ...
      <div className="flex flex-col w-[1000px] ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col mt-5">
            <IspaidComponent isPaid={order!.isPaid}/>
            ...
            {order!.OrderItem.map((item) => (
              <div className="flex" key={item.prodcut.slug}>
                <Image
                  src={`/products/${item.prodcut.productImage[0].url}`}
                  width={100}
                  height={100}
                ...
                <div>
                  <p>{item.prodcut.title}-{item.size}</p>
                  <p>${item.price} x {item.quantity}</p>
                  <p className="font-bold">Sutotal: $ {item.price * item.quantity}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl p-7">
            ...
            {
              !order?.isPaid ?
                (<PaypalButtons orderId={order!.id} amount={order!.total} />):
                ( <IspaidComponent isPaid={order!.isPaid}/>)
            }
          </div>
        </div>
